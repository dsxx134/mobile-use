...
from minitap.mobile_use.collectors.xianyu_collector.models import BitBrowserConfig
...
def test_build_collector_service_uses_saved_bitbrowser_config_when_env_is_missing(tmp_path, monkeypatch):
    db = CollectorDatabase(tmp_path / "collector.db")
    db.initialize()
    repo = AppConfigRepository(db)
    repo.save_saved_cookie_string("")
    ...
    monkeypatch.delenv("XIANYU_COLLECTOR_BITBROWSER_ID", raising=False)
    monkeypatch.delenv("BIT_BROWSER_ID", raising=False)
    monkeypatch.setattr(
        "minitap.mobile_use.collectors.xianyu_collector.runtime.SavedCookieProvider",
        FakeSavedCookieProvider,
    )
