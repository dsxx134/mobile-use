...
from minitap.mobile_use.scenarios.xianyu_publish.flow import (
    ...
    extract_listing_coin_discount_value,
)
from minitap.mobile_use.scenarios.xianyu_publish.models import TapTarget
...
def test_detects_listing_form_coin_discount_and_shipping_time_entries():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {"content-desc": "价格设置", "bounds": "[70,1635][1530,1775]"},
            {"content-desc": "发货方式\n包邮", "bounds": "[70,1775][1530,1915]"},
            {"content-desc": "闲鱼币抵扣\n30%", "bounds": "[70,1915][1530,2055]"},
            {"content-desc": "发货时间\n24小时发货", "bounds": "[70,2055][1530,2195]"},
        ],
    )
    ...
    assert analysis.screen_name == "listing_form"
    assert analysis.targets["coin_discount_entry"].text == "闲鱼币抵扣\n30%"
    assert analysis.targets["shipping_time_entry"].text == "发货时间\n24小时发货"
...
def test_detects_listing_form_without_add_image_when_scrolled():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {
                "content-desc": "描述, 描述一下宝贝的品牌型号、货品来源…",
                "bounds": "[30,190][1570,469]",
            },
            {"content-desc": "价格设置", "bounds": "[70,1161][1530,1301]"},
            {"content-desc": "发货方式\n包邮", "bounds": "[70,1441][1530,1581]"},
            {"content-desc": "选择位置", "bounds": "[70,1581][1530,1721]"},
        ],
    )
    ...
    assert analysis.screen_name == "listing_form"
    assert "add_image" not in analysis.targets


def test_detects_listing_form_shipping_time_toggles():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {"content-desc": "选择位置", "bounds": "[70,1581][1530,1721]"},
            {
                ...
                "content-desc": "24小时发货\n承诺在买家付款后24小时内发货",
                "checked": "false",
                "bounds": "[30,2098][1570,2271]",
            },
            {
                "class": "android.widget.Switch",
                "content-desc": "48小时发货\n承诺在买家付款后48小时内发货",
                "checked": "true",
                "bounds": "[30,2271][1570,2445]",
            },
        ],
    )
    ...
    assert analysis.screen_name == "listing_form"
    assert "shipping_time_toggle_24小时发货" in analysis.targets
    assert "shipping_time_toggle_48小时发货" in analysis.targets
    assert "shipping_time_toggle_selected_48小时发货" in analysis.targets


def test_detects_listing_form_shipping_time_toggle_variants():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {
                "class": "android.widget.Switch",
                "content-desc": "24小时内发货\n承诺在买家付款后24小时内发货",
                "checked": "true",
                "bounds": "[30,2098][1570,2271]",
            },
            {
                "class": "android.widget.Switch",
                "content-desc": "48小时内发货\n承诺在买家付款后48小时内发货",
                "checked": "false",
                "bounds": "[30,2271][1570,2445]",
            },
        ],
    ...
    assert "shipping_time_toggle_24小时发货" in analysis.targets
    assert "shipping_time_toggle_48小时发货" in analysis.targets
    assert "shipping_time_toggle_selected_24小时发货" in analysis.targets
...
def test_detects_listing_form_metadata_entry_target_from_more_info_variant():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {"content-desc": "分类, 更多信息\n分类", "bounds": "[0,1494][1600,1815]"},
            {"content-desc": "已选中电子资料, 电子资料", "bounds": "[245,1615][435,1755]"},
            {"content-desc": "价格设置", "bounds": "[70,1957][1530,2097]"},
            {"content-desc": "发货方式\n包邮", "bounds": "[70,2237][1530,2377]"},
            {"content-desc": "选择位置", "bounds": "[70,2377][1530,2517]"},
        ],
    )
    ...
    assert analysis.screen_name == "listing_form"
    assert analysis.targets["metadata_entry"].text == "分类, 更多信息\n分类"
    assert analysis.targets["metadata_entry"].x == 800
    assert analysis.targets["metadata_entry"].y == 1654


def test_listing_form_metadata_entry_skips_item_spec_row():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            ...
            {"content-desc": "商品规格\n非必填，设置多个颜色、尺码等", "bounds": "[30,1494][1570,1634]"},
            {"content-desc": "价格设置", "bounds": "[70,1957][1530,2097]"},
            {"content-desc": "发货方式\n包邮", "bounds": "[70,2237][1530,2377]"},
            {"content-desc": "选择位置", "bounds": "[70,2377][1530,2517]"},
        ],
    ...
    assert analysis.screen_name == "listing_form"
    assert "metadata_entry" not in analysis.targets
...
def test_detects_location_panel_without_region_entry():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
    ...
    assert analysis.targets["location_back"].x == 100
    assert analysis.targets["location_search"].text == "搜索地址"
    assert analysis.targets.get("location_region_entry") is None
...
def test_detects_publish_success_screen_without_actions():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"content-desc": "发布成功", "bounds": "[698,1028][1000,1105]"},
            {"content-desc": "奖励加码！发布抵扣宝贝得", "bounds": "[113,1156][830,1229]"},
        ],
    )
    ...
    assert analysis.screen_name == "publish_success"
    assert "publish_success_view_listing" not in analysis.targets
    assert "publish_success_continue" not in analysis.targets
...
def test_detects_network_error_dialog_targets():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"text": "亲，无法连接网络，请检查网络！", "bounds": "[0,2025][1600,2560]"},
            {"text": "我知道了", "clickable": "true", "bounds": "[40,2355][1560,2455]"},
        ],
    )

    analysis = analyzer.detect_screen(screen)

    assert analysis.screen_name == "network_error_dialog"
    assert analysis.targets["network_error_ack"].text == "我知道了"
...
def test_detects_coin_discount_panel_with_option_targets():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"text": "设置闲鱼币抵扣", "bounds": "[720,1161][1000,1209]"},
            {"text": "30%", "bounds": "[40,1400][200,1460]"},
            {"text": "20%", "bounds": "[40,1510][200,1570]"},
            {"text": "10%", "bounds": "[40,1620][200,1680]"},
            {"text": "开启抵扣", "bounds": "[760,2381][840,2429]"},
        ],
    )

    analysis = analyzer.detect_screen(screen)

    assert analysis.screen_name == "coin_discount_panel"
    assert analysis.targets["coin_discount_option_30"].text == "30%"
    assert analysis.targets["coin_discount_option_20"].text == "20%"
    assert analysis.targets["coin_discount_option_10"].text == "10%"
    assert analysis.targets["coin_discount_enable"].text == "开启抵扣"


def test_detects_shipping_time_panel_with_option_targets():
    analyzer = XianyuFlowAnalyzer()
    screen = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"text": "发货时间", "bounds": "[720,1161][880,1209]"},
            {"text": "24小时发货", "bounds": "[40,1400][260,1460]"},
            {"text": "48小时发货", "bounds": "[40,1510][260,1570]"},
            {"text": "确定", "bounds": "[760,2381][840,2429]"},
        ],
    )
    ...
    assert analysis.screen_name == "shipping_time_panel"
    assert analysis.targets["shipping_time_option_24小时发货"].text == "24小时发货"
    assert analysis.targets["shipping_time_option_48小时发货"].text == "48小时发货"
    assert analysis.targets["shipping_time_confirm"].text == "确定"
...
def test_tap_target_rejects_ai_assistant_entry():
    android_service = FakeAndroidService(screens=[_make_screen(elements=[])])
    flow = XianyuPublishFlowService(
        settings=XianyuPublishSettings(),
        android_service=android_service,
    ...
    with pytest.raises(RuntimeError, match="Forbidden AI assistant target"):
        flow._tap_target(  # noqa: SLF001
            "device-1",
            TapTarget(
                x=480,
                y=1475,
                text="AI帮你写, AI帮你润色",
                bounds="[70,1410][1530,1540]",
            ),
            "description_entry",
        )
...
def test_advance_to_listing_form_recovers_from_network_error_dialog():
    publish_chooser = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {
                "content-desc": "发闲置\n自己拍图卖·啥都能换钱",
                "bounds": "[0,1500][1600,1723]",
                "clickable": "true",
            },
            {"content-desc": "关闭", "bounds": "[0,0][120,120]"},
        ],
    )
    network_error = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"text": "亲，无法连接网络，请检查网络！", "bounds": "[0,2025][1600,2560]"},
            {"text": "我知道了", "clickable": "true", "bounds": "[40,2355][1560,2455]"},
    ...
    android_service = FakeAndroidService(
        screens=[publish_chooser, network_error, publish_chooser, listing_form],
        advance_on_tap_calls={1, 2, 3},
    )
    flow = XianyuPublishFlowService(
        settings=XianyuPublishSettings(),
...
def test_advance_to_listing_form_dismisses_system_overlay():
    overlay_screen = {
        "serial": "device-1",
        "platform": "android",
        "current_app": {
            ...
            "package": None,
            "activity": None,
            "raw_output": "mCurrentFocus=Window{64f855 u0 NotificationShade}",
        },
        "width": 2560,
        "height": 1600,
        "base64": "abc123",
        "hierarchy_xml": "<hierarchy />",
        "elements": [{"text": "上滑解锁", "bounds": "[0,0][200,100]"}],
        "element_count": 1,
    }
    listing_form = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    android_service = FakeAndroidService(
        screens=[overlay_screen, listing_form],
        advance_on_get_indices={0},
    )
    flow = XianyuPublishFlowService(
        settings=XianyuPublishSettings(),
        android_service=android_service,
    ...
    assert result.screen_name == "listing_form"
    assert call("input keyevent KEYCODE_BACK") in android_service.device.shell.call_args_list
    assert call("input keyevent KEYCODE_HOME") in android_service.device.shell.call_args_list
...
def test_search_location_and_select_result_with_value_returns_selected_text():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    result, selected_location = flow.search_location_and_select_result_with_value(
        "device-1",
        "上海虹桥站",
    )

    assert result.screen_name == "listing_form"
    assert selected_location == "上海虹桥站 新虹街道申贵路1500号"
...
def test_advance_listing_form_to_location_search_screen_recovers_from_publish_chooser():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    result = flow.advance_listing_form_to_location_search_screen("device-1")

    assert result.screen_name == "location_search_screen"
    assert android_service.tap_calls == [
        ("device-1", 1920, 761),
        ("device-1", 800, 1985),
        ("device-1", 800, 245),
...
def test_advance_listing_form_to_metadata_panel_waits_through_listing_form_tail():
    listing_form = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"content-desc": "关闭", "bounds": "[0,105][90,165]"},
            ...
            _make_screen(
                ...
                elements=[
                    ...
                    },
                    {
                        "content-desc": (
                            "分类/品牌/计价方式\n"
                            "当前分类不支持修改服务信息，如需设置可从「首页-卖闲置-发服务」进行发布"
                        ),
                        "bounds": "[0,1695][1600,2397]",
                    },
                    {"content-desc": "分类, 分类", "bounds": "[40,1867][215,1907]"},
                    {
                        "content-desc": "已选中软件安装包/序列号/激活码, 软件安装包/序列号/激活码",
                        "bounds": "[245,1817][690,1957]",
                    },
                    {"content-desc": "可选电子资料, 电子资料", "bounds": "[710,1817][900,1957]"},
                    {"content-desc": "价格设置", "bounds": "[70,2538][1530,2560]"},
                ],
            ),
        ],
    ...
    assert result.screen_name == "metadata_panel"
    assert android_service.tap_calls == [("device-1", 800, 1654)]


def test_advance_listing_form_to_coin_discount_panel_reveals_entry():
    listing_form_top = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"content-desc": "关闭", "bounds": "[0,105][90,165]"},
            {"content-desc": "发闲置", "bounds": "[90,104][255,166]"},
    ...
    listing_form_bottom = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"content-desc": "关闭", "bounds": "[0,105][90,165]"},
            {"content-desc": "发闲置", "bounds": "[90,104][255,166]"},
            ...
            {"content-desc": "价格设置", "bounds": "[70,1161][1530,1301]"},
            {"content-desc": "闲鱼币抵扣\n30%", "bounds": "[70,1301][1530,1441]"},
            {"content-desc": "发货方式\n包邮", "bounds": "[70,1441][1530,1581]"},
            {"content-desc": "选择位置", "bounds": "[70,1581][1530,1721]"},
        ],
    )
    coin_panel = _make_screen(
        activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
        elements=[
            {"text": "设置闲鱼币抵扣", "bounds": "[720,1161][1000,1209]"},
            {"text": "30%", "bounds": "[40,1400][200,1460]"},
    ...
    android_service = FakeAndroidService(
        screens=[listing_form_top, listing_form_bottom, coin_panel],
        advance_on_swipe_calls={1},
        advance_on_tap_calls={1},
    )
    flow = XianyuPublishFlowService(
    ...
    result = flow.advance_listing_form_to_coin_discount_panel("device-1")

    assert result.screen_name == "coin_discount_panel"
    assert android_service.swipe_calls == [
        ("device-1", 800, 2099, 800, 896, 350),
    ]
    assert android_service.tap_calls == [("device-1", 800, 1371)]


def test_advance_listing_form_to_metadata_panel_waits_through_loading_overlay():
    overlay_screen = _make_screen(
        activity=(
            "com.idlefish.flutterbridge.flutterboost.boost."
            "FishFlutterBoostTransparencyActivity"
        ),
        elements=[],
    )
    listing_form = _make_screen(
...
def test_require_location_written_on_editor_raises_when_location_is_unset():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
                elements=_scrolled_metadata_panel_elements(),
            ),
        ]
    )
    flow = XianyuPublishFlowService(
    ...
    with pytest.raises(RuntimeError, match="Location is still unset on editor"):
        flow.require_location_written_on_editor("device-1")


def test_require_location_written_on_editor_accepts_metadata_panel_without_location_entry():
    elements = [
        element
        for element in _scrolled_metadata_panel_elements()
        if element.get("content-desc") != "选择位置"
    ]
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
    ...
    result = flow.require_location_written_on_editor("device-1")

    assert result.screen_name == "metadata_panel"


def test_require_location_written_on_editor_accepts_selected_location_when_entry_still_present():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    result = flow.require_location_written_on_editor(
        "device-1",
        selected_location="上海虹桥站",
    )
...
def test_fill_description_accepts_listing_form_after_selector_input():
    description = "测试描述123"
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
...
def test_set_coin_discount_selects_option_and_returns():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    result = flow.set_coin_discount("device-1", "30%")
...
def test_set_shipping_time_selects_option_and_returns():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
                elements=[
                    ...
                    {"content-desc": "价格设置", "bounds": "[70,1635][1530,1775]"},
                    {"content-desc": "发货方式\n包邮", "bounds": "[70,1775][1530,1915]"},
                    {"content-desc": "发货时间\n48小时发货", "bounds": "[70,1915][1530,2055]"},
                ],
            ),
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
            ...
            _make_screen(
                ...
                elements=[
                    ...
                    {"content-desc": "价格设置", "bounds": "[70,1635][1530,1775]"},
                    {"content-desc": "发货方式\n包邮", "bounds": "[70,1775][1530,1915]"},
                    {"content-desc": "发货时间\n24小时发货", "bounds": "[70,1915][1530,2055]"},
                ],
            ),
        ],
        advance_on_tap_calls={1, 2},
    ...
    result = flow.set_shipping_time("device-1", "24小时发货")
...
def test_set_shipping_time_uses_toggle_when_present():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    assert result.screen_name == "listing_form"
    assert android_service.tap_calls == [("device-1", 800, 2184)]


def test_set_coin_discount_selects_card_and_enables_when_text_options_missing():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
            ...
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
                elements=[
                    {"content-desc": "设置闲鱼币抵扣", "bounds": "[600,520][1000,620]"},
                    {"class": "android.widget.HorizontalScrollView", "bounds": "[0,1200][1600,1840]"},
                    {"content-desc": "开启抵扣", "bounds": "[600,2150][1000,2240]"},
                ],
            ),
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
...
def test_set_coin_discount_accepts_allowed_status_row_without_percent_text():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
                elements=[
                    ...
                    {"content-desc": "价格设置", "bounds": "[70,1635][1530,1775]"},
                    {"content-desc": "发货方式\n包邮", "bounds": "[70,1775][1530,1915]"},
                    {"content-desc": "闲鱼币抵扣", "bounds": "[70,1915][1530,2055]"},
                ],
            ),
        ],
    )
    ...
    result = flow._analyze("device-1")

    assert result.screen_name == "listing_form"
    assert (
        extract_listing_coin_discount_value(result.targets.get("coin_discount_entry"))
        == "on"
    )


def test_extract_listing_coin_discount_value_reads_percent_from_single_line():
    target = TapTarget(x=10, y=10, text="闲鱼币抵扣 允许买家使用闲鱼币支付30%")

    assert extract_listing_coin_discount_value(target) == "30%"
...
def test_extract_listing_receipt_from_activity_dump_parses_item_id_and_deep_link():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
    ...
    receipt = flow.extract_listing_receipt_from_activity_dump("device-1")
...
def test_extract_listing_receipt_from_activity_dump_returns_none_when_dump_has_no_detail_link():
    android_service = FakeAndroidService(
        screens=[
            _make_screen(
                activity="com.idlefish.flutterbridge.flutterboost.boost.FishFlutterBoostActivity",
