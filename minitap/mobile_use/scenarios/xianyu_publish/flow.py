...
_COIN_DISCOUNT_PERCENT_RE = re.compile(r"(?P<percent>\d+%)")
_SHIPPING_TIME_HOURS_RE = re.compile(r"(?P<hours>24|48)\s*小时")
...
def normalize_stock_input(stock: str | float | int) -> str:
    raw = str(stock).strip()
    if not raw:
        raise ValueError("Stock input cannot be empty")
    if "." in raw or "。" in raw:
        raise ValueError(f"Stock input must be an integer: {stock!r}")
    if raw.startswith("-"):
        raise ValueError("Stock input cannot be negative")
    if not raw.isdigit():
        raise ValueError(f"Stock input must be an integer: {stock!r}")
    return str(int(raw))
...
def normalize_coin_discount(discount: str) -> str:
    raw = str(discount).strip()
    if not raw:
        raise ValueError("Coin discount cannot be empty")
    ...
    normalized = aliases.get(raw)
    if normalized is None:
        raise ValueError(f"Unsupported coin discount: {discount!r}")
    return normalized


def extract_listing_coin_discount_value(target: TapTarget | None) -> str | None:
    if target is None or not target.text:
        return None
    match = _COIN_DISCOUNT_PERCENT_RE.search(target.text)
    if match is not None:
        return match.group("percent")
    lines = [line.strip() for line in target.text.splitlines() if line.strip()]
    if not lines:
        return None
    if "闲鱼币抵扣" in lines[0] and len(lines) >= 2:
        return lines[1]
    if "闲鱼币抵扣" in lines[0] and len(lines) == 1:
        return "on"
    return None


def normalize_shipping_time(value: str) -> str:
    raw = str(value).strip()
    if not raw:
        raise ValueError("Shipping time cannot be empty")

    match = _SHIPPING_TIME_HOURS_RE.search(raw)
    if match is not None:
    ...
    normalized = aliases.get(raw)
    if normalized is None:
        raise ValueError(f"Unsupported shipping time: {value!r}")
    return normalized


def extract_listing_shipping_time_value(target: TapTarget | None) -> str | None:
    if target is None or not target.text:
        return None
    lines = [line.strip() for line in target.text.splitlines() if line.strip()]
    if not lines:
        return None
    if len(lines) >= 2 and any(marker in lines[0] for marker in ("发货时间", "发货时效")):
        return lines[1]
    return None
...
class XianyuFlowAnalyzer:
    ...
    def _find_shipping_time_toggle_targets(
        self,
        elements: Iterable[dict],
    ) -> dict[str, TapTarget]:
        ...
        for element in elements:
            ...
            if not text:
                continue
            if "发货时间" in text or "发货时效" in text:
                continue
            match = _SHIPPING_TIME_HOURS_RE.search(text)
            if match is None:
                continue
            hours = match.group("hours")
            label = f"{hours}小时发货"
            ...
            if not suffix:
                continue
            targets[f"shipping_time_toggle_{suffix}"] = target.model_copy(update={"text": label})
            checked = element.get("checked")
            selected = element.get("selected")
            if (
                checked is True
                or str(checked).lower() == "true"
                or selected is True
                or str(selected).lower() == "true"
                or "已选" in text
            ):
                targets[f"shipping_time_toggle_selected_{suffix}"] = target.model_copy(
                    update={"text": label}
                )
        return targets
    ...
    def _find_simple_option_targets(
        self,
        elements: Iterable[dict],
        *,
        prefix: str,
        ...
        for option in options:
            target = self._find_target(elements, exact_text=option)
            if target is None and header_text is not None:
                target = self._find_multiline_option_target(
                    elements,
            ...
            if not suffix:
                continue
            targets[f"{prefix}_option_{suffix}"] = target.model_copy(update={"text": option})
        return targets
    ...
    def _find_horizontal_scroll_area_target(
        self,
        elements: Iterable[dict],
    ) -> TapTarget | None:
        best_target: TapTarget | None = None
        best_area = 0
        for element in elements:
            element_class = element.get("class") or element.get("className")
            if element_class != "android.widget.HorizontalScrollView":
                continue
            target = parse_android_bounds(element.get("bounds"))
            if target is None or not isinstance(target.bounds, str):
                continue
            match = _BOUNDS_RE.fullmatch(target.bounds)
            if match is None:
                continue
            left = int(match.group("left"))
            top = int(match.group("top"))
        ...
        if best_target is None:
            return None
        return best_target.model_copy(update={"text": "coin_discount_cards"})
    ...
    def detect_screen(self, screen_data: dict) -> XianyuScreenAnalysis:
        ...
        if (
            ...
            if publish_success_continue is not None:
        ...
        network_error_ack = self._find_target(elements, exact_text="我知道了")
        if (
            current_package == self._settings.xianyu_package_name
            and any("无法连接网络" in text for text in visible_texts)
            and network_error_ack is not None
        ):
            targets["network_error_ack"] = network_error_ack
            return XianyuScreenAnalysis(
                screen_name="network_error_dialog",
                current_package=current_package,
                current_activity=current_activity,
                visible_texts=visible_texts,
                targets=targets,
        ...
        coin_discount_entry = self._find_target(elements, contains_text="闲鱼币抵扣")
        shipping_time_entry = self._find_target(elements, contains_text="发货时间") or self._find_target(
            elements, contains_text="发货时效"
        )
        location_entry = self._find_target(elements, exact_text="选择位置")
        shipping_time_toggle_targets = self._find_shipping_time_toggle_targets(elements)
        ...
        original_price_entry = self._find_target(elements, exact_text="原价设置")
        stock_entry = self._find_target(elements, exact_text="库存设置")
        if (
            current_package == self._settings.xianyu_package_name
            ...
            if original_price_entry is not None:
                targets["original_price_entry"] = original_price_entry
            if stock_entry is not None:
                targets["stock_entry"] = stock_entry
        ...
        coin_discount_confirm = self._find_target(elements, exact_text="确定")
        coin_discount_enable = self._find_target(elements, exact_text="开启抵扣")
        coin_discount_cards = self._find_horizontal_scroll_area_target(elements)
        coin_discount_targets = self._find_simple_option_targets(
            elements,
            prefix="coin_discount",
            options=("30%", "20%", "10%"),
            header_text="闲鱼币抵扣",
        )
        coin_discount_panel_texts = ("设置闲鱼币抵扣", "闲鱼币抵扣比例", "闲鱼币抵扣")
        if (
            current_package == self._settings.xianyu_package_name
            and (
                len(coin_discount_targets) >= 2
                or coin_discount_enable is not None
                or coin_discount_cards is not None
            )
            and any(
                any(marker in text for marker in coin_discount_panel_texts)
                for text in visible_texts
            )
        ):
            targets.update(coin_discount_targets)
            if coin_discount_confirm is not None:
                targets["coin_discount_confirm"] = coin_discount_confirm
            if coin_discount_enable is not None:
                targets["coin_discount_enable"] = coin_discount_enable
            if coin_discount_cards is not None:
                targets["coin_discount_cards"] = coin_discount_cards
            return XianyuScreenAnalysis(
                screen_name="coin_discount_panel",
                current_package=current_package,
                current_activity=current_activity,
                visible_texts=visible_texts,
                targets=targets,
        ...
        shipping_time_confirm = self._find_target(elements, exact_text="确定")
        shipping_time_targets = self._find_simple_option_targets(
            elements,
            prefix="shipping_time",
            options=("24小时发货", "48小时发货"),
            header_text="发货时间",
        )
        if (
            current_package == self._settings.xianyu_package_name
            and len(shipping_time_targets) >= 2
            and any("发货时间" in text for text in visible_texts)
        ):
            targets.update(shipping_time_targets)
            if shipping_time_confirm is not None:
                targets["shipping_time_confirm"] = shipping_time_confirm
            return XianyuScreenAnalysis(
                screen_name="shipping_time_panel",
                current_package=current_package,
                current_activity=current_activity,
                visible_texts=visible_texts,
                targets=targets,
        ...
        if (
            ...
            if coin_discount_entry is not None:
                targets["coin_discount_entry"] = coin_discount_entry
            if shipping_time_entry is not None:
                targets["shipping_time_entry"] = shipping_time_entry
            targets.update(shipping_time_toggle_targets)
        ...
        if (
            current_package == self._settings.xianyu_package_name
            and any(text == "发闲置" for text in visible_texts)
            and (add_image is not None or description_entry is not None)
            and any("价格设置" in text for text in visible_texts)
...
class XianyuPublishFlowService:
    ...
    def _reveal_listing_form_target(
        self,
        serial: str,
        *,
        target_name: str,
        ...
        analysis = self._analyze(serial)
        if (
            analysis.screen_name != "listing_form"
            or analysis.targets.get(target_name) is not None
        ):
            return analysis
    ...
    def _dismiss_system_overlays(self, serial: str) -> None:
        device = self._android.get_device(serial)
    ...
    def advance_to_listing_form(
        ...
        for _ in range(max_steps):
            ...
            if analysis.current_package != self._settings.xianyu_package_name:
                if analysis.current_package is None or analysis.current_package == "com.android.systemui":
                    self._dismiss_system_overlays(serial)
                self.open_home(serial)
                continue

            if analysis.screen_name == "unknown" and self._looks_like_loading_overlay(analysis):
                analysis = self._wait_for_non_loading_screen(serial)
                if analysis.screen_name in {"listing_form", "metadata_panel", "unknown"}:
                    continue
            ...
            if analysis.screen_name == "network_error_dialog":
                target = analysis.targets.get("network_error_ack")
                if target is None:
                    raise RuntimeError("Missing network_error_ack target on network error dialog")
                self._tap_target(serial, target, "network_error_ack")
                analysis = self._wait_for_non_loading_screen(serial)
                network_error_hits += 1
                if network_error_hits >= 2:
                    raise RuntimeError(
                        "Network error dialog persisted while opening Xianyu publish form"
                    )
                if analysis.screen_name in {"listing_form", "metadata_panel", "publish_chooser"}:
                    continue
                continue
    ...
    def advance_listing_form_to_coin_discount_panel(
        self,
        serial: str,
        max_steps: int = 4,
    ) -> XianyuScreenAnalysis:
        for _ in range(max_steps):
            analysis = self._analyze(serial)
            if analysis.screen_name == "coin_discount_panel":
                return analysis

            if analysis.screen_name in {"listing_form", "metadata_panel"}:
                target = analysis.targets.get("coin_discount_entry")
                if target is None and analysis.screen_name == "listing_form":
                    analysis = self._reveal_listing_form_target(
                        serial,
                        target_name="coin_discount_entry",
                        max_swipes=3,
                    )
                    target = analysis.targets.get("coin_discount_entry")
                if target is None and analysis.screen_name == "metadata_panel":
                    analysis = self._reveal_metadata_target(
                        serial,
                        target_name="coin_discount_entry",
                        max_swipes=3,
                ...
                if target is None:
                    raise RuntimeError(
                        f"Missing coin_discount_entry target on {analysis.screen_name}"
                    )
                self._tap_target(serial, target, "coin_discount_entry")
                analysis = self._wait_for_screen_transition(
                    serial,
                    transient_screen_names={"listing_form", "metadata_panel"},
                )
                if analysis.screen_name in {
                    "coin_discount_panel",
                    "unknown",
            ...
            raise RuntimeError(
                "Cannot reach coin discount panel from current screen: "
                f"{analysis.screen_name}"
            )

        final_analysis = self._analyze(serial)
        if final_analysis.screen_name == "coin_discount_panel":
            return final_analysis
        raise RuntimeError(
            "Failed to reach coin discount panel within "
            f"{max_steps} steps: {final_analysis.screen_name}"
        )

    def advance_listing_form_to_shipping_time_panel(
        self,
        serial: str,
        max_steps: int = 4,
    ) -> XianyuScreenAnalysis:
        for _ in range(max_steps):
            analysis = self._analyze(serial)
            if analysis.screen_name == "shipping_time_panel":
                return analysis

            if analysis.screen_name in {"listing_form", "metadata_panel"}:
                target = analysis.targets.get("shipping_time_entry")
                if target is None and analysis.screen_name == "listing_form":
                    analysis = self._reveal_listing_form_target(
                        serial,
                        target_name="shipping_time_entry",
                        max_swipes=3,
                    )
                    target = analysis.targets.get("shipping_time_entry")
                if target is None and analysis.screen_name == "metadata_panel":
                ...
                if target is None:
                    raise RuntimeError(
                        f"Missing shipping_time_entry target on {analysis.screen_name}"
                    )
                self._tap_target(serial, target, "shipping_time_entry")
                analysis = self._wait_for_non_loading_screen(serial)
                if analysis.screen_name in {"shipping_time_panel", "unknown"}:
                    continue

            raise RuntimeError(
                "Cannot reach shipping time panel from current screen: "
                f"{analysis.screen_name}"
            )

        final_analysis = self._analyze(serial)
        if final_analysis.screen_name == "shipping_time_panel":
            return final_analysis
        raise RuntimeError(
            "Failed to reach shipping time panel within "
            f"{max_steps} steps: {final_analysis.screen_name}"
        )
    ...
    def fill_price(
        ...
        return self.fill_price_panel_fields(
            serial,
            price=price,
            max_steps=max_steps,
            max_clear_presses=max_clear_presses,
            key_delay_seconds=key_delay_seconds,
        )

    def _fill_price_panel_value(
        self,
        serial: str,
        analysis: XianyuScreenAnalysis,
        *,
        value_text: str,
        entry_target_name: str | None = None,
        ...
        if analysis.screen_name != "price_panel":
            raise RuntimeError(
                f"Expected price_panel before filling price value, got {analysis.screen_name}"
            )

        if entry_target_name:
            entry_target = analysis.targets.get(entry_target_name)
            if entry_target is None:
                raise RuntimeError(
                    f"Missing {entry_target_name} target on price panel"
                )
            self._tap_target(serial, entry_target, entry_target_name)
            analysis = self._wait_for_non_loading_screen(serial)
            if analysis.screen_name != "price_panel":
                raise RuntimeError(
                    "Price panel entry tap did not stay on price panel: "
                    f"{analysis.screen_name}"
                )
        ...
        for char in value_text:
            if char == "." and not allow_decimal:
                raise RuntimeError(
                    "Decimal input is not allowed for this price panel field"
                )
            target_name = "price_key_decimal" if char == "." else f"price_key_{char}"
            key_target = targets.get(target_name)
            if key_target is None:
                raise RuntimeError(
                    f"Missing keypad target for price character: {char!r}"
                )
        ...
        final_panel = self._analyze(serial)
        if final_panel.screen_name == "price_panel":
            return final_panel
        return analysis

    def fill_price_panel_fields(
        self,
        serial: str,
        *,
        price: str | float | int,
        original_price: str | float | int | None = None,
        stock: str | float | int | None = None,
        max_steps: int = 4,
        max_clear_presses: int = 8,
        key_delay_seconds: float = 0.2,
    ) -> XianyuScreenAnalysis:
        ...
        if original_text is not None:
            analysis = self._fill_price_panel_value(
                ...
                entry_target_name="original_price_entry",
                max_clear_presses=max_clear_presses,
                key_delay_seconds=key_delay_seconds,
            )
        if stock_text is not None:
            analysis = self._fill_price_panel_value(
                serial,
                analysis,
                value_text=stock_text,
                entry_target_name="stock_entry",
                allow_decimal=False,
                max_clear_presses=max_clear_presses,
                key_delay_seconds=key_delay_seconds,
            )

        confirm_target = analysis.targets.get("price_confirm")
        if confirm_target is None:
    ...
    def set_coin_discount(
        self,
        serial: str,
        discount: str,
        max_steps: int = 4,
    ) -> XianyuScreenAnalysis:
        desired_discount = normalize_coin_discount(discount)
        current_analysis = self._analyze(serial)
        if current_analysis.screen_name in {"listing_form", "metadata_panel"}:
            current_value = extract_listing_coin_discount_value(
                current_analysis.targets.get("coin_discount_entry")
            )
            if current_value == desired_discount:
                return current_analysis

        analysis = self.advance_listing_form_to_coin_discount_panel(serial, max_steps=max_steps)
        target_name = {
            "30%": "coin_discount_option_30",
            "20%": "coin_discount_option_20",
            "10%": "coin_discount_option_10",
        }[desired_discount]
        option_target = analysis.targets.get(target_name)
        if option_target is not None:
            self._tap_target(serial, option_target, target_name)
        else:
            card_area = analysis.targets.get("coin_discount_cards")
            if card_area is None or not isinstance(card_area.bounds, str):
                raise RuntimeError(f"Missing {target_name} target on coin discount panel")
            match = _BOUNDS_RE.fullmatch(card_area.bounds)
            if match is None:
                raise RuntimeError("Invalid coin discount card bounds")
            left = int(match.group("left"))
            top = int(match.group("top"))
            right = int(match.group("right"))
            bottom = int(match.group("bottom"))
            ...
            index = {"30%": 0, "20%": 1, "10%": 2}[desired_discount]
            x = left + int((index + 0.5) * (width / 3))
            y = top + int(height * 0.35)
            self._android.tap(serial, x, y)

        updated_panel = self._wait_for_non_loading_screen(serial)
        if updated_panel.screen_name == "coin_discount_panel":
            analysis = updated_panel

        enable_target = analysis.targets.get("coin_discount_enable") or updated_panel.targets.get(
            "coin_discount_enable"
        )
        if enable_target is not None:
            self._tap_target(serial, enable_target, "coin_discount_enable")
            updated_panel = self._wait_for_non_loading_screen(serial)
        else:
            confirm_target = analysis.targets.get("coin_discount_confirm") or updated_panel.targets.get(
                "coin_discount_confirm"
            )
            if confirm_target is not None:
                self._tap_target(serial, confirm_target, "coin_discount_confirm")
                updated_panel = self._wait_for_non_loading_screen(serial)
            else:
                raise RuntimeError("Missing coin discount confirm/enable target")

        if updated_panel.screen_name not in {"listing_form", "metadata_panel"}:
            raise RuntimeError(
                "Coin discount flow did not return to a supported editor screen: "
                f"{updated_panel.screen_name}"
            )

        final_value = extract_listing_coin_discount_value(
            updated_panel.targets.get("coin_discount_entry")
        )
        if final_value != desired_discount:
            retry_analysis = self._analyze(serial)
            fallback_value = None
            for candidate in retry_analysis.targets.values():
                candidate_value = extract_listing_coin_discount_value(candidate)
                if candidate_value is not None:
                    fallback_value = candidate_value
                    break
            if fallback_value not in {desired_discount, "on"}:
                raise RuntimeError(
                    "Coin discount flow ended on "
                    f"{fallback_value!r} instead of {desired_discount!r}"
                )
    ...
    def set_shipping_time(
        ...
        desired_time = normalize_shipping_time(shipping_time)
        current_analysis = self._analyze(serial)
        toggle_key = f"shipping_time_toggle_{desired_time}"
        selected_toggle_key = f"shipping_time_toggle_selected_{desired_time}"
        if selected_toggle_key in current_analysis.targets:
            return current_analysis
        toggle_target = current_analysis.targets.get(toggle_key)
        if toggle_target is not None:
            self._tap_target(serial, toggle_target, toggle_key)
            updated = self._wait_for_non_loading_screen(serial)
            if selected_toggle_key in updated.targets:
                return updated
            refreshed = self._analyze(serial)
            if selected_toggle_key in refreshed.targets:
                return refreshed
            raise RuntimeError(
                "Shipping time toggle did not switch to "
                f"{desired_time!r} on listing form"
            )
        if current_analysis.screen_name in {"listing_form", "metadata_panel"}:
            current_value = extract_listing_shipping_time_value(
                current_analysis.targets.get("shipping_time_entry")
            )
            if current_value == desired_time:
                return current_analysis

        analysis = self.advance_listing_form_to_shipping_time_panel(serial, max_steps=max_steps)
        target_name = {
            "24小时发货": "shipping_time_option_24小时发货",
            "48小时发货": "shipping_time_option_48小时发货",
        }[desired_time]
        option_target = analysis.targets.get(target_name)
        if option_target is None:
            raise RuntimeError(f"Missing {target_name} target on shipping time panel")
        self._tap_target(serial, option_target, target_name)

        updated_panel = self._wait_for_non_loading_screen(serial)
        if updated_panel.screen_name == "shipping_time_panel":
            analysis = updated_panel

        confirm_target = analysis.targets.get("shipping_time_confirm") or updated_panel.targets.get(
            "shipping_time_confirm"
        )
        if confirm_target is not None:
            self._tap_target(serial, confirm_target, "shipping_time_confirm")
            updated_panel = self._wait_for_non_loading_screen(serial)

        if updated_panel.screen_name not in {"listing_form", "metadata_panel"}:
            raise RuntimeError(
                "Shipping time flow did not return to a supported editor screen: "
                f"{updated_panel.screen_name}"
            )

        final_value = extract_listing_shipping_time_value(
            updated_panel.targets.get("shipping_time_entry")
        )
        if final_value != desired_time:
            raise RuntimeError(
                f"Shipping time flow ended on {final_value!r} instead of {desired_time!r}"
            )
        return updated_panel
