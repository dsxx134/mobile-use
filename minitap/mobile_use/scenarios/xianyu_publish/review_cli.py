...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mobile-use-xianyu-review",
        description="Prepare and review one publishable Xianyu listing from Feishu without submitting it."
    )
    parser.add_argument(
        "--serial",
...
def run_review_cli(
    ...
    *,
    ...
    result = prepare_fn(
        ...
        auto_publish_after_prepare=False,
    )
    return format_live_prepare_result(result)


def main(argv: Sequence[str] | None = None) -> None:
    print(run_review_cli(argv))
