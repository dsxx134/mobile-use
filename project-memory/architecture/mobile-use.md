...
- The collector config surface now also has a bridging command between strong live sessions and saved state:
...
- The listing form now also exposes optional rows for `闲鱼币抵扣` and `发货时间`.
  These are treated as best-effort inputs, only applied when the Bitable fields are non-empty.
  The analyzer only classifies their bottom-sheet panels when multiple options are visible, to
  avoid misclassifying the listing-form row as a panel.
- The `发货时间` row can appear as `发货时效` and can be off-screen in the listing form or metadata
  panel. The flow now scrolls those editors to reveal the row before opening the shipping-time
  panel, and the runner logs a skip if the row is still missing for the current category.
- The metadata entry target on the listing form must always come from a `分类` row and explicitly
  ignores `商品规格` rows to avoid opening the spec editor.
- The metadata-panel bridge now waits through transient `unknown` loading overlays (the transparent
  Flutter boost activity) before tapping the `分类` entry.
- The preheat path now handles Xianyu's `无法连接网络` dialog by tapping `我知道了` before retrying
  the publish flow.
- Listing-form detection no longer requires the `添加图片` tile to be visible; a scrolled form that
  still shows description plus `价格设置/发货方式` should remain classified as `listing_form`.
- Shipping-time selection also supports toggle-only layouts (`24小时发货/48小时发货`) without a
  `发货时间` header, by tapping the desired switch directly on the listing form.
- When the device is on a system overlay like the notification shade (no app package), the flow
  now dismisses the overlay before opening Xianyu during preheat.
- The coin-discount entry row can be off-screen; the flow now scrolls the editor to reveal
  `闲鱼币抵扣` before opening the discount panel.
- The coin-discount sheet (`设置闲鱼币抵扣`) renders options as cards without text in the UI
  hierarchy. The flow taps by card index within the `HorizontalScrollView` bounds and then
  presses `开启抵扣` to apply the selection.
