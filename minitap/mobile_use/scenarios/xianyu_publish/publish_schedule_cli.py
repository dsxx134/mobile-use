...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mobile-use-xianyu-publish-schedule",
        description="Run the live Xianyu batch worker on a fixed interval for scheduler-friendly production use."
    )
    parser.add_argument(
        "--serial",
...
def run_publish_schedule_cli(
    argv: Sequence[str] | None = None,
    *,
    schedule_fn: Callable[..., XianyuLiveQueueScheduleResult] = run_publish_queue_schedule_live,
) -> tuple[str, int]:
    ...
    output = format_live_queue_schedule_result(result)
    exit_code = 2 if result.total_failure_count > 0 else 0
    return output, exit_code


def main(argv: Sequence[str] | None = None) -> None:
    output, exit_code = run_publish_schedule_cli(argv)
    print(output)
