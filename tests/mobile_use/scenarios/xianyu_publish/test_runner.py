...
def test_prepare_first_publishable_listing_runs_feishu_media_and_flow_chain_in_order(tmp_path):
    ...
    flow.fill_price_panel_fields.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    timeline = Mock()
    ...
    assert timeline.mock_calls == [
        ...
        call.flow.fill_price_panel_fields(
            "device-1",
            price=199.0,
            original_price=None,
            stock=None,
...
def test_prepare_first_publishable_listing_applies_optional_fields_when_present(tmp_path):
    listing = _make_listing(
        coin_discount="30%",
        shipping_method="包邮",
    ...
    flow.fill_description.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.fill_price_panel_fields.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.set_coin_discount.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.set_shipping_method.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.set_shipping_time.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    runner = XianyuPrepareRunner(
        source=source,
        media_sync=media_sync,
        flow=flow,
    ...
    flow.set_coin_discount.assert_called_once_with("device-1", "30%")
    flow.set_shipping_method.assert_called_once_with("device-1", "包邮")
    flow.set_shipping_time.assert_called_once_with("device-1", "24小时发货")


def test_prepare_first_publishable_listing_skips_optional_fields_when_missing(tmp_path):
    listing = _make_listing()
    source = Mock()
    source.pick_first_publishable_record.return_value = listing
    source.get_attachment_download_urls.return_value = {"ft-1": "https://files.example/1.jpg"}
    ...
    flow.set_coin_discount.assert_not_called()
    flow.set_shipping_method.assert_not_called()
    flow.set_shipping_time.assert_not_called()


def test_prepare_first_publishable_listing_skips_shipping_time_when_entry_missing(tmp_path):
    listing = _make_listing(shipping_time="24小时发货")
    source = Mock()
    source.pick_first_publishable_record.return_value = listing
    source.get_attachment_download_urls.return_value = {"ft-1": "https://files.example/1.jpg"}
    media_sync = Mock()
    ...
    flow.fill_description.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.fill_price_panel_fields.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.set_shipping_time.side_effect = RuntimeError(
        "Missing shipping_time_entry target on listing_form"
    )
    runner = XianyuPrepareRunner(
        source=source,
        media_sync=media_sync,
...
def test_prepare_first_publishable_listing_accepts_metadata_panel_after_image_flow(tmp_path):
    ...
    flow.fill_price_panel_fields.return_value = XianyuScreenAnalysis(screen_name="metadata_panel")
    runner = XianyuPrepareRunner(
        source=source,
        media_sync=media_sync,
        flow=flow,
...
def test_prepare_first_publishable_listing_marks_failure_and_reraises(tmp_path):
    ...
    flow.select_cover_image.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.fill_description.return_value = XianyuScreenAnalysis(screen_name="listing_form")
    flow.fill_price_panel_fields.side_effect = RuntimeError("price panel missing")
    runner = XianyuPrepareRunner(
        source=source,
        media_sync=media_sync,
        flow=flow,
